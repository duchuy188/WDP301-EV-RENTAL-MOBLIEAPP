...
export const bookingAPI = {
  ...
  // Check booking status by temp_id (for polling after payment)
  checkBookingStatus: async (tempId: string) => {
    const response = await apiClient.get(`/bookings/check/${tempId}`);
    return response.data;
  },
